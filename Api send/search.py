...
def fctn(input):
    ...
    sound = AudioSegment.from_file(input,format="wav", codec="aac")
...
def call_database_for_answer(type_of_answer, datas):
    ...
    if type_of_answer == "datetime":  #daca dorim un raspuns ce ne indica timpul (ex: I:"cand am fost la facultate", R:<data><ora>)
        sql_select = "SELECT ORA, DATE1, LOCATIE FROM LOCATIE where LOWER(LOCATIE) like '%" + datas.lower() + "%'"  #avem grija sa convertim atat stringul gasit in intrebare cat si cel gasit in db la litere mici

    if type_of_answer == "location": #daca dorim un raspuns ce ne indica locatia (ex: I:"unde am fost ieri", R:<locatie>)
        if datas[1] == "00:00:00":
            sql_select = "SELECT ORA, DATE1, LOCATIE FROM LOCATIE where date1 = '" + datas[0] + "'"
        else:
            sql_select = "SELECT ORA, DATE1, LOCATIE FROM LOCATIE where date1 = '" + datas[0] + "'and ora like '" + datas[1] + "%'"
...
# mp3Save('./uploads/mesaj-audio.mp3')